...
Deno.serve(async (req) => {
  ...
  try {
    const { recipientId, amount, currency = "USD", memo } = await req.json();
    ...
    const payoutData = {
      ...
      memo: memo || `Payout for ${amount} ${currency}`,
    };

    const response = await fetch("https://api.trolley.com/v1/payouts", {
      method: "POST",
      headers: {
    ...
    console.error("Error processing Trolley payout:", error);
